<div class="mx-auto max-w-7xl space-y-6">
    ...
    <x-college.filter-card cols="3">
        <div>
            ...
            <x-text-input id="search" type="text" class="mt-1 block w-full text-sm" placeholder="{{ __('Search materials...') }}" wire:model.live.debounce.300ms="search" />
        </div>
        <div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Material details') }}</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($rows as $row)
                        <tr wire:key="mat-{{ $row->id }}">
                            <td class="px-6 py-4 text-sm">
                                <div class="font-medium text-gray-900 dark:text-white">{{ $row->title }}</div>
                                @if($row->description)
                                    <div class="text-xs text-gray-500 dark:text-gray-400 max-w-sm truncate">{{ $row->description }}</div>
                                @endif
                                @if ($row->file_path)
                                    <div class="mt-1 flex items-center gap-1.5 text-xs text-indigo-650 dark:text-indigo-400 font-medium">
                                        <i class="fa-solid fa-paperclip"></i>
                                        <a href="{{ route('teacher.courses.materials.download', $row->id) }}" target="_blank" class="hover:underline">
                                            {{ __('Download File') }} ({{ strtoupper($row->file_type ?? 'FILE') }})
                                        </a>
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-right text-sm">
                                <div class="flex justify-end gap-3">
                                    ...
                                    <button
                                        ...
                                        title="{{ __('Delete') }}"
                                        class="text-red-600 hover:text-red-950 dark:text-red-400 dark:hover:text-red-300"
                                    >
    ...
    <!-- Review / Audit Modal -->
    @if ($showReviewModal && $selectedMaterial)
        <x-college.modal name="mat-review" :title="__('Review Course Material')" :show="true" maxWidth="lg" livewireSynced>
            <div class="space-y-4">
                <div>
                    <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Material details') }}</h3>
                    <p class="text-base font-medium text-gray-900 dark:text-white mt-1">{{ $selectedMaterial->title }}</p>
                    <p class="text-sm text-gray-600 dark:text-gray-300 mt-1 whitespace-pre-line">{{ $selectedMaterial->description ?: __('No description provided.') }}</p>
                </div>

                <div class="grid grid-cols-2 gap-4 border-y border-gray-150 py-3 dark:border-gray-700">
                    <div>
                        <span class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">{{ __('Lecturer') }}</span>
                        <span class="text-sm text-gray-800 dark:text-gray-200 font-medium">{{ $selectedMaterial->teacher?->lastname }} {{ $selectedMaterial->teacher?->othernames }}</span>
                    </div>
                    <div>
                        <span class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">{{ __('Course') }}</span>
                        <span class="text-sm text-gray-800 dark:text-gray-200 font-medium">{{ $selectedMaterial->course?->code }}</span>
                    </div>
                </div>

                @if ($selectedMaterial->file_path)
                    <div class="rounded-lg bg-gray-50 dark:bg-gray-900/50 p-3 flex items-center justify-between border border-gray-200 dark:border-gray-700">
                        <div class="flex items-center gap-2">
                            <i class="fa-solid fa-file-pdf text-red-500 fa-xl"></i>
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Attached File') }}</span>
                        </div>
                        <a href="{{ route('teacher.courses.materials.download', $selectedMaterial->id) }}" target="_blank" class="rounded-md bg-white border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-750 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-250 dark:border-gray-600 dark:hover:bg-gray-700">
                ...
                <div>
                    ...
                    <textarea
                        ...
                        class="mt-1 block w-full rounded-md border-gray-350 dark:border-gray-650 dark:bg-gray-900 dark:text-white text-sm"
                    ></textarea>
            ...
            <x-slot:footer>
                ...
                <div class="flex gap-2">
                    <button type="button" wire:click="rejectMaterial" class="rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-500">
                        <i class="fa-solid fa-circle-xmark mr-1"></i> {{ __('Reject') }}
                    </button>
                    <button type="button" wire:click="approveMaterial" class="rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-500">
    ...
    @if ($showDeleteModal)
        <x-college.modal name="mat-delete" :title="__('Delete Course Material?')" :show="true" maxWidth="md" livewireSynced>
            <p class="text-sm text-gray-600 dark:text-gray-400">
                {{ __('Are you sure you want to permanently delete this course material? This action cannot be undone.') }}
            </p>
            <x-slot:footer>
